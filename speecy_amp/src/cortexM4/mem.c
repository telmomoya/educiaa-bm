...
byte readmem(word dir){
		...
		int valor;
		if (dir < 0x4000) valor = spectrum_rom[dir];
		else
			if (dir<0x6000) valor = ScreenArea[dir-0x4000];
			else valor = RAM_SPECTRUM(dir-0x6000);
...
void writemem(word dir, byte data){
	...
	if (dir >= 0x4000)
		{
		if (dir<0x6000) ScreenArea[dir-0x4000]=data;
		else RAM_SPECTRUM(dir-0x6000) = data;
